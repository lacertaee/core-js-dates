...
function isDateInPeriod(date, period) {
  ...
  const ins = new Date(date);
  if (start.getTime() <= ins.getTime() && ins.getTime() <= end.getTime()) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  ...
  return `${month}/${days}/${year}, ${hours}:${mins}:${secs} ${per}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const answ = (ms / 86400000 + toMon) / 7;
  if (answ % 1 === 0) {
    return answ + 1;
  }
  return Math.ceil(answ);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const temp = period.end.split('-');
  const [start, end] = [
    new Date(period.start),
    new Date(temp[2], temp[1] - 1, temp[0]),
  ];
  const func = (date) => {
    const d = new Date(date);
    const days = String(d.getDate()).padStart(2, '0');
    const months = String(d.getMonth() + 1).padStart(2, '0');
    const answ = `${days}-${months}-${start.getFullYear()}`;
    return answ;
  };
  const arr = [];
  while (start.getTime() + countOffDays <= end.getTime()) {
    if (countWorkDays > 1) {
      for (let i = 0; i < countWorkDays; i += 1) {
        if (arr.length === 0) {
          arr.push(func(start));
        } else {
          arr.push(func(start.setDate(start.getDate() + 1)));
        }
      }
      ...
      arr.push(func(start.setDate(start.getDate() + countOffDays + 1)));
    }
  }
  return arr.filter((x) => {
    const t = x.split('-');
    const check = new Date(t[2], t[1] - 1, t[0]);
    return check <= end;
  });
}
